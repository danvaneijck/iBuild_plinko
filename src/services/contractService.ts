import { MsgExecuteContractCompat, MsgBroadcaster, getInjectiveAddress } from '@injectivelabs/sdk-ts';
...
export class ContractService {
  ...
  async getPlinkBalance(address: string): Promise<string> {
    try {
      ...
      const response = await fetch(
        `${endpoints.rest}/cosmwasm/wasm/v1/contract/${CONTRACTS.plinkToken}/smart/${btoa(JSON.stringify(queryMsg))}`
      );
      ...
      return new BigNumberInBase(balance).dividedBy(new BigNumberInBase(10).pow(18)).toFixed(2);
    } catch (error) {
  ...
  async purchasePlink(injAmount: string, userAddress: string): Promise<any> {
    try {
      ...
      const amount = new BigNumberInBase(injAmount).times(new BigNumberInBase(10).pow(18)).toFixed(0);
  ...
  async getPurchaseConfig(): Promise<any> {
    try {
      ...
      const response = await fetch(
        `${endpoints.rest}/cosmwasm/wasm/v1/contract/${CONTRACTS.purchase}/smart/${btoa(JSON.stringify(queryMsg))}`
      );
   ...
   */
  async approvePlinkSpending(amount: string, userAddress: string): Promise<any> {
    try {
      const injectiveAddress = getInjectiveAddress(userAddress);
      
      // Convert amount to base units
      const baseAmount = new BigNumberInBase(amount).times(new BigNumberInBase(10).pow(18)).toFixed(0);
  ...
  async getPlinkAllowance(userAddress: string): Promise<string> {
    try {
      ...
      return new BigNumberInBase(allowance).dividedBy(new BigNumberInBase(10).pow(18)).toFixed(2);
    } catch (error) {
  ...
  async playGame(
    ...
    try {
      ...
      const baseAmount = new BigNumberInBase(betAmount).times(new BigNumberInBase(10).pow(18)).toFixed(0);
  ...
  async getGameStats(): Promise<any> {
    try {
      ...
      const response = await fetch(
        `${endpoints.rest}/cosmwasm/wasm/v1/contract/${CONTRACTS.game}/smart/${btoa(JSON.stringify(queryMsg))}`
      );
   ...
   */
  async getGameHistory(userAddress: string, limit: number = 20): Promise<any> {
    try {
      const injectiveAddress = getInjectiveAddress(userAddress);
      const queryMsg = {
        history: {

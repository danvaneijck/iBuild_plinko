...
export const useContracts = (userAddress: string) => {
  ...
  const contractService = useMemo(() => {
    return new ContractService(walletStrategy);
  }, []);
  ...
  }, [userAddress, contractService, contractsValid]);
  ...
  const fetchGameHistory = useCallback(async () => {
    ...
    try {
      const history = await contractService.getGameHistory(userAddress, 20);
  ...
  const purchasePlink = useCallback(
    async (injAmount: string) => {
      ...
      try {
        const result = await contractService.purchasePlink(injAmount, userAddress);
    ...
    [userAddress, contractService, fetchPlinkBalance, contractsValid]
  );
  ...
  const playGame = useCallback(
    ...
    [userAddress, plinkBalance, contractService, fetchPlinkBalance, fetchGameHistory, contractsValid]
  );
